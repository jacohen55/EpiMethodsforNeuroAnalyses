...
## Pulling in phenotypic data from Rundle
```{r}
# Pulling in VAT and covariate data sent over by Rundle 
Rundle <- read_excel(str_c(gdrive,"Jacob/Obesity_Project/data_code/","P50_reduced_Data_N300_MI_data_03102022_NoImpute.xlsx")) %>% 
            dplyr::select(-Imputation_) 

Rundle[,-1][Rundle[,-1] == -9] <- NA 
Rundle[,-1][Rundle[,-1] == 9] <- NA 
Rundle[,-1][Rundle[,-1] == 8] <- NA 

Rundle[,c("p50_ethnicity","p50_cohabitate","p50_Poor_SRH", "p50_Housing", "A11Y11", "A12Y11", "A13Y11", "A14Y11", "A15Y11", "A17Y11")] <- as.data.frame(lapply(Rundle[,c("p50_ethnicity","p50_cohabitate","p50_Poor_SRH", "p50_Housing", "A11Y11", "A12Y11", "A13Y11", "A14Y11", "A15Y11", "A17Y11")], function(x) as.factor(x)))

glimpse(Rundle)
nrow(Rundle) #300
sapply(Rundle, levels)
...
xy <- read_sav(str_c(gdrive,"MN_DATA /GENDER.sav")) %>% 
            rename(sid = SID) %>% mutate(GENDER = as.factor(GENDER))
...
P50_imputs_Rundle25 <- read.csv(str_c(gdrive,"Jacob/Obesity_Project/data_code/","P50_reduced_Data_N300_MI_data_03102022.csv")) %>% 
...
noImput <- c(ANTHRO_names, brain_names, bchart_names, prenat_names, c("sid", "SIBID", "VAT_check", "Thick_check", "ID", "subID"), CKLIST16_names)
...
noPred <- c(ANTHRO_names, brain_namesnoPred, bchart_names, prenat_names, c("sid", "SIBID", "VAT_check", "Thick_check", "ID", "subID"), CKLIST16_names)
...
# Figures not working when you run as chunk becuase of Google Drive configuration - need to run from command line
...
model3 <- lm(data = P50, rh_G_oc.temp_med.Parahip_thickness ~ VAT + p50_ethnicity + p50_cohabitate + p50_education + p50_Poor_SRH + 
                                                                  ...
                                                                  A14Y11 + A15Y11 + A17Y11 + BirthWt + age_mri_yr + GENDER) #+ BMUSCLE + BSAT + BIMA +
...
tmpdf1v <- drop_na(braindf[,c("subID","BMIZ11","age_mri_yr", "GENDER")])
nrow(tmpdf1v)

l.1v.model <- qdecr_fastlm(formula = qdecr_thickness ~ BMIZ11 + GENDER + age_mri_yr , # VAT*GENDER --> nothing came out 
                    data = tmpdf1v,
                    id = "subID",
                    hemi = "lh",
                    project = "1v",
                    ...
                    fwhm = 10,
                    dir_subj = SUBJECTS_DIR ,dir_fshome = FREESURFER_HOME, n_cores = 4,save = T,verbose = T,save_data = T, cwp_thr = 0.5)

r.1v.model <- qdecr_fastlm(formula = qdecr_thickness ~ VAT + GENDER + age_mri_yr , # VAT*GENDER --> nothing came out
                    data = tmpdf1v,
                    id = "subID",
                    hemi = "rh",
                    project = "1v",
...
tmpdf20v <- drop_na(braindf[,c("subID","VAT", "p50_ethnicity", "p50_cohabitate",
                              ...
                              "A14Y11", "A15Y11", "A17Y11", "BirthWt", "age_mri_yr", "GENDER")]) # removing to help with collinearity "BSAT", "BIMA", , "BMUSCLE"
...
l.20v.model <- qdecr_fastlm(formula = qdecr_thickness ~ VAT + p50_ethnicity + p50_cohabitate + p50_education + p50_Poor_SRH +
                                                                  SES12 + SES13 + p50_Housing + tfi_score + A11Y11 + A12Y11 + A13Y11 +
                                                                  A14Y11 + A15Y11 + A17Y11 + BirthWt + age_mri_yr + GENDER, #+ BSAT + BIMA+ BMUSCLE  
                    data = tmpdf20v,
                    id = "subID",
                    hemi = "lh",
                    project = "20v",
...
tmpdf20v.mids <- P50_imputs %>% mice::filter(subID %in% braindf$subID) # this applies the check for fsavg.mgh file above on all imputed datasets 
...
#summary(r.1v.model, annot = TRUE)
summary(l.1v.model, annot = TRUE)
hist(P50$VAT)
hist(P50$BMUSCLE)

summary(r.2v.w.model, annot = TRUE)
summary(l.2v.w.model, annot = TRUE)
# 
# summary(r.20v.w.model, annot = TRUE)
# summary(r.20v.w.model, annot = TRUE)

#summary(r.20v.model, annot = TRUE)
summary(l.20v.model, annot = TRUE)

#summary(r.20v.mids.model, annot = TRUE)
summary(l.20v.mids.model, annot = TRUE)

# Run from command line. Graphics stuff not running in chunk. 
# qdecr_snap(l.1v.model, 'VAT', ext = "_1v.tiff")
# qdecr_snap(l.20v.model, 'VAT', ext = "_20v.tiff")
# qdecr_snap(l.20v.mids.model, 'VAT', ext = "_20vMIDS.tiff")
...
```{r}
vw <- qdecr_load("/Users/jacob/Desktop/QDECR/lh.1v.thickness/") # loads in qdecr object
vw <- reload(vw) # not sure what this step does but seems to be necessary 
str(vw$mgh) # contains thickness, rows are vertices, columns are SIDs
vw$stack # tells us which stack goes with which regressors
clusters <- qdecr_read_ocn(vw, "BMIZ11")$x #extracts vector with cluster number for each vertex in stack 1
str(clusters)
table(clusters)
c1 <- clusters == 1
row.mask <- as.logical(c1)
row.ind = bigstatsr::rows_along(vw$mgh)
row.ind <- row.ind[row.mask]

fbm_col_mean(vw$mgh, row.mask = c1) # gets mean thickness for each participant in cluster 1 
vw$input$mgh_ids
vw$md

clusters_spec <- qdecr_read_ocn(vw, "BMIZ11")
qdecr_read_coef(vw, "BMIZ11")
qdecr_read_p(vw, "BMIZ11")
qdecr_read_t(vw, "BMIZ11")
qdecr_read_se(vw, "BMIZ11")
qdecr_read_ocn_mask(vw, "BMIZ11")
```
